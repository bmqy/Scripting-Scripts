...
## 2026-08-15 RSS 源筛选显示未读数验证报告

- 数据依据：FeedOverview 已包含 unreadCount，且由现有源概览加载、已读扣减和刷新流程维护。
...
[2026-08-15 16:10 Asia/Shanghai] 发布核对：版本字段仅修改项目与 RSS 组件，未修改限号组件；dev 已同步远端，待提交后合并到 main。

## [2026-09-21 通过本地时间] 倒班排班小组件验证

- 构建：npm run build 成功，新增包内四个文件均存在；构建物随后按仓库约定清理。
- 日期模型：起始日当天使用所选起始班次，日期偏移使用 UTC 日序差，跨月/跨年和向前查询断言通过。
- 配置边界：少于两个班次、非法日期、越界起始班次会回退或裁剪到安全可展示配置；Storage 不可用时保存明确返回失败。
- UI 结构：小号显示今日班次，中号显示今日和前五天，大号显示未来七天；锁屏圆形/矩形均有单独渲染路径。
...
## [2026-09-22 通过本地时间] 倒班排班布局紧凑化验证

- 组件层级：中号由“标题 + 独立说明 + 日期条 + 底部说明”收敛为“标题栏 + 日期条”，大号由“标题 + 日历 + 独立今日面板 + 说明”收敛为“标题栏 + 月历 + 更新说明”。
- 尺寸约束：复用既有周条带的固定列尺寸思路；大号每个日期格固定高度，空白日期格与有效日期格保持一致。
- 验证结果：最终构建通过，差异检查通过；未进行真实 iOS Host 渲染验证。
