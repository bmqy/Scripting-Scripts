...
- OPML 模式支持本地保存文章已读状态，默认使用 Storage；也可在“组件配置”中切换到 SQLite。切换存储方式时会自动迁移已有状态
...
OPML 已读状态只保存在当前设备。SQLite 数据库使用 Scripting 的 App Group 文档目录，Storage 和 SQLite 两种方式都不会修改原始 OPML 文件。
