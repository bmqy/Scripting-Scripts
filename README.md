...
按自定义的倒班周期显示今天和未来日期的班次，适合白班、夜班、休息等固定轮换安排。界面采用日历式信息层级，参考了[“小渔排班”公开资料](https://appshunter.io/ios/app/selene-shift-calendar/id1410053842)中强调的日历样式、主题和 Widget 体验，但不依赖其数据或素材。

#### 功能特点

- 在脚本设置页选择起始日期和起始班次
- 用“、”或逗号填写 2—8 项循环班次，例如 `白班、夜班、休息`
- 小号突出今天，中号显示近期班次，大号显示未来 7 天
- 支持锁屏圆形和矩形小组件
- 设置保存后立即请求刷新，小组件在每天 00:05 后请求进入下一天

#### 使用方法

1. 导入 `dist/倒班排班.scripting`。
2. 在 Scripting App 中运行“倒班排班”脚本。
3. 设置起始日期、起始班次和循环班次，点击“保存排班并刷新小组件”。
4. 将 Scripting App 的“倒班排班”小组件添加到主屏幕或锁屏。

小组件实际更新时间仍由 iOS 系统调度；如果修改了排班，重新运行脚本保存即可触发刷新。
...
- 提供 Scripting App 的主页默认 UI 入口：进入后直接显示默认 RSS 源文章列表，左侧源按钮可切换源，中间保持显示“RSS 阅读”，右上角可筛选文章，下拉可刷新文章，滚动触底自动加载下一页，最后一页可切换下一个源，滚动离开文章后自动标记已读；首次使用前需先运行脚本完成配置
